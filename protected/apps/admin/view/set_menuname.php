                ...
                foreach($list as $vo){
					...
					if($vo['rootid']!=0){
                       if($vo['pid']==0)
                          ...
                          $menus.='<div class="group_con" title="'.$vo['operate'].'"><input style="display:none" name="menu[]" '.$check.' class="check'.$vo['pid'].'" type="checkbox" value="'.$vo['id'].'" /><input class="cname" name="mname['.$vo['id'].']"  type="text" value="'.$vo['name'].'" /></div>';
                    }
				}
                echo $menus;
             }
