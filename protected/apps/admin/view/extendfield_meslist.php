...
function lock(obj){
	     obj.click(function(){
			...
			$.post("{url('extendfield/meslock')}", {id:id,tabid:"{$tableid}",ispass:0},
   				function(data){
					if(data==1){
                      nowobj.html("审核");
...
function unlock(obj){
		obj.click(function(){
			var nowobj=$(this);
			var id=nowobj.parent().parent().attr('id');
			$.post("{url('extendfield/meslock')}", {id:id,tabid:"{$tableid}",ispass:1},
   				function(data){
					if(data==1){
                      nowobj.html("取消");
...
<div class="list_head_m">        
           <div class="list_head_ml">当前位置：【自定义表中数据】</div>
</div>                           
          ...
          <?php 
                 if(!empty($list)){
                      foreach($list as $vo){
						  ...
						  for($i=1;$i<count($tableinfo);$i++){
							  $cont.= '<td align="center">'.html_out($vo[$tableinfo[$i]['tableinfo']]).'</td>';
						  }
