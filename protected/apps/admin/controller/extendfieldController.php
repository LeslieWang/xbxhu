...
class extendfieldController extends commonController
{
	...
	public function tableedit()
	{
		if (!$this->isPost()) {
			...
			if(model('extend')->update($condition,$data))
			$this->success('拓展表修改成功~');
			else $this->error('拓展表修改失败~');
		}
	}
	...
	public function fieldadd()
	{
		if (!$this->isPost()) {
			...
			$data['norder'] = in($_POST['norder']); //排序
	...
	public function file()
	{
		...
		if (!$this->isPost()) {
			...
			echo '<form action="'.url('extendfield/file').'" method="post" style=" margin:0; padding:0;"  enctype="multipart/form-data" name="form1" id="form1">';
	...
	{
		$pid=intval($_GET['id']);
        $tableinfo = model('extend')->select("id='{$pid}' OR pid='{$pid}'",'id,tableinfo,name','pid');
